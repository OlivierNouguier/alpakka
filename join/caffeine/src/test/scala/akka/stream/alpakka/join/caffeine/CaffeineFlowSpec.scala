 ...
 */
package akka.stream.alpakka.join.caffeine

import akka.actor.ActorSystem
import akka.stream.alpakka.join.Aggregator
import akka.stream.alpakka.join.caffeine.internal.CaffeineStore
...
class CaffeineTestSuite extends WordSpec with Matchers {
  ...
  "CaffeineFanOutSage" should {
    "aggregate events in flow" in {
      ...
      val f = source(to)
        .via(
          AggregatorFlow(aggregator) { expired =>
            new CaffeineStore[String, AggregatedEvents](expired, 100 millis)
          }
        )
      ...
      expiredCount shouldEqual to - to / 3
    ...
    "aggregate event in graph" in {
      ...
      val g = Source.fromGraph(GraphDSL.create() { implicit builder =>
        ...
        val reco = builder.add(
          AggregatorFlow.tee(aggregator) { expired =>
            new CaffeineStore[String, AggregatedEvents](expired, 100 millis)
          }
        )
        source(10) ~> reco.in
